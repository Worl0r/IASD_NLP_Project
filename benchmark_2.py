...
def train_and_evaluate(model, train_loader, val_loader, optimizer, scheduler, loss_fn, writer, device, num_epochs=5, model_type="transformer", seq_len=64):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for i, batch in enumerate(tqdm(train_loader, desc=f"{model_type}-{seq_len} Training Epoch {epoch}")):
            ...
            train_acc += acc
            writer.add_scalar(f"Loss/train_{model_type}_{seq_len}", loss, epoch * len(train_loader) + i)
            writer.add_scalar(f"Accuracy/train_{model_type}_{seq_len}", acc, epoch * len(train_loader) + i)
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader, desc=f"{model_type}-{seq_len} Validation Epoch {epoch}"):
                input_ids = batch["input_ids"].to(device)
                labels = batch["label"].to(device)
        ...
        # Log to tensorboard
        writer.add_scalar(f"Loss/train_epoch_{model_type}_{seq_len}", avg_train_loss, epoch)
        writer.add_scalar(f"Accuracy/train_epoch_{model_type}_{seq_len}", avg_train_acc, epoch)
        writer.add_scalar(f"Loss/val_epoch_{model_type}_{seq_len}", avg_val_loss, epoch)
        writer.add_scalar(f"Accuracy/val_epoch_{model_type}_{seq_len}", avg_val_acc, epoch)
        
        print(f"[{model_type}-{seq_len} Epoch {epoch}] "
...
def compare_seq_lengths(num_epochs=5):
    ...
    results = {
        "transformer": {length: {"summary": {}, "epochs": []} for length in seq_lengths},
        "linformer": {length: {"summary": {}, "epochs": []} for length in seq_lengths}
    }
    ...
    for seq_len in seq_lengths:
        ...
        transformer_metrics = train_and_evaluate(
            ...
            writer, device, num_epochs=num_epochs, model_type="transformer", seq_len=seq_len
        )
        ...
        results["transformer"][seq_len]["summary"] = {
            ...
            "total_time": sum(epoch["total_time"] for epoch in transformer_metrics),
        ...
        linformer_metrics = train_and_evaluate(
            model, train_loader, val_loader, optimizer, scheduler, loss_fn, 
            writer, device, num_epochs=num_epochs, model_type="linformer", seq_len=seq_len
        )
        ...
        results["linformer"][seq_len]["summary"] = {
            ...
            "total_time": sum(epoch["total_time"] for epoch in linformer_metrics),
            "max_memory": torch.cuda.max_memory_allocated(device) / (1024 ** 2)  # MB
        }
    
    # Print results summary
    print("\n=== Results Summary ===")
    print("Sequence Length | Model      | Val Acc | Total Time (s) | Memory (MB)")
    print("----------------|------------|---------|----------------|------------")
    for seq_len in seq_lengths:
        t_summary = results["transformer"][seq_len]["summary"]
        l_summary = results["linformer"][seq_len]["summary"]
        
        print(f"{seq_len:14} | Transformer | {t_summary['final_val_acc']:.4f} | {t_summary['total_time']:.2f} | {t_summary['max_memory']:.2f}")
        print(f"{seq_len:14} | Linformer   | {l_summary['final_val_acc']:.4f} | {l_summary['total_time']:.2f} | {l_summary['max_memory']:.2f}")
        ...
        print(f"{seq_len:14} | Comparison  | Δ={acc_diff:.4f} | Speedup={speedup:.2f}x | Mem ratio={memory_ratio:.2f}x")
        print("----------------|------------|---------|----------------|------------")
    ...
    for model_type in ["transformer", "linformer"]:
        for seq_len in seq_lengths:
            ...
            with open(f'benchmark_results/{model_type}_seq{seq_len}_epochs.json', 'w') as f:
